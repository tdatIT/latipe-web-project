                ...
                <div class="container-xxl flex-grow-1 container-p-y">
                    ...
                    <div class="card">
                        <div class="card-header border-bottom">
                            ...
                            >
                                <div class="order_status col-md-4">
                                    <select id="FilterTransaction" class="form-select text-capitalize">
                                        <option value="All" ${option=="0" ? ' selected="selected"' : ''}>All</option>
                                        <option value="Active" ${option=="1" ? ' selected="selected"' : ''}>Processing
                                        </option>
                                        <option value="Inactive" ${option=="2" ? ' selected="selected"' : ''}> Delivery
                                        </option>
                                        <option value="Inactive" ${option=="3" ? ' selected="selected"' : ''}> Success
                                        </option>
                                        <option value="Inactive" ${option=="4" ? ' selected="selected"' : ''}> Cancel
                                        </option>
                                    </select>
                                </div>
                            </div>
                        ...
                        <div class="card-datatable table-responsive">
                            ...
                            >
                                ...
                                <!-- table -->
                                <table
                                        class="datatables-orders table border-top dataTable no-footer dtr-column collapsed"
                                    ...
                                    <c:forEach var="item" items="${orders}">
                                        <tr class="odd">
                                            <td class="sorting_1"><a
                                                    href="link toi profile :))">${item.orderId}</a></td>
                                            <td>
                                                <div class="d-flex justify-content-start align-items-center">
                                                    ...
                                                    <div class="d-flex flex-column">
                                                        ...
                                                        ><span class="fw-semibold">${item.storeByStoreId.name}</span>
                                                        </a
                                                        >
                                                    </div>
                                                </div>
                                            ...
                                            <td>
                                                <div class="d-flex justify-content-start align-items-center">
                                                    <div class="avatar-wrapper">
                                                        <div class="avatar avatar-sm me-3">
                                                            <img
                                                                    src="${pageContext.request.contextPath}/upload/${item.userByUserId.avatar}"
                                                                    alt="Avatar"
                                                                    class="rounded-circle"
                                                            />
                                                        </div>
                                                    ...
                                                    <div class="d-flex flex-column">
                                                        <a
                                                                href="link toi profile :))"
                                                                class="text-body text-truncate"
                                                        ><span class="fw-semibold">${item.userByUserId.firstname} ${item.userByUserId.lastname}</span>

                                                            <small class="text-muted">${item.userByUserId.email}</small>
                                                        </a
                                                        >
                                                    </div>
                                                </div>
                                            ...
                                            <td>
                                                ...
                                                >${item.deliveryByDeliveryId.name}</span>
                                            </td>
                                            <td>
                                                <span class="fw-semibold"
                                                >${String.format("%.0f", item.amountFromUser)}</span>
                                            </td>
                                            <td>
                                                <span class="fw-semibold"
                                                >${String.format("%.0f", item.amountFromStore)}</span>
                                            </td>
                                            <td>
                                                <span class="fw-semibold"
                                                >${String.format("%.0f", item.amountToStore)}</span>
                                            </td>
                                            <td>
                                                <span class="fw-semibold"
                                                >${String.format("%.0f", item.amountToGd)}</span>
                                            </td>
                                            <td>
                                                ${item.isPaidBefore() != false ?'<span class="badge bg-label-success"> Paid</span>' : '<span class="badge bg-label-secondary"> Debt</span>'}
                                            </td>
                                            <td>
                                                <c:if test="${item.status == 1}">
                                                    <span class="badge bg-label-success"> Processing</span>
                                                </c:if>
                                                ...
                                                <c:if test="${item.status == 4}">
                                                    <span class="badge bg-label-success"> Cancel</span>
                                                </c:if>
                                            </td>
                                                ...
                                                <div class="d-inline-block">
                                                    ...
                                                    <div class="dropdown-menu dropdown-menu-end">
                                                        <c:if test="${item.status == 1}">
                                                            ...
                                                            href="order/active?orderId=${item.orderId}&status=2"
                                                            >Delivery</a
                                                            >
                                                            <a
                                                                    class="dropdown-item text-success"
                                                                    href="order/active?orderId=${item.orderId}&status=3"
                                                            >Success</a
                                                            >
                                                            <a
                                                                    class="dropdown-item text-success"
                                                                    href="order/active?orderId=${item.orderId}&status=4"
                                                            >User Cancel</a
                                                            >
                                                            <a
                                                                    class="dropdown-item text-success"
                                                                    href="order/active?orderId=${item.orderId}&status=5"
                                                            >Shop Cancel</a
                                                            >
                                                        </c:if>
                                                        <c:if test="${item.status == 2}">
                                ...
                                <!-- pagination -->
                                <div id="paginate-order" class="row mx-2 mt-2">
                                    <div class="col-12">
                                                ...
                                                <ul class="pagination">
                                                    ...
                                                    <c:forEach begin="0" end="${endPage}" var="i">
                                                        <li class="page-item paginate_button ${tag==i ? "active" : ""}">
                                                            <a
                                                                    class="page-link"
                                                                    href="transaction?page=${i}">${i+1}</a>
                                                        </li>
                                                    </c:forEach>
                                                    <c:if test="${tag<endPage}">
                                                        <li class="page-item page-item next"><a class="page-link"
                                                                                                href="transaction?page=${tag+1}">Next</a>
                                                        </li>
                                                    </c:if>
                                                </ul>
                                            </div>
